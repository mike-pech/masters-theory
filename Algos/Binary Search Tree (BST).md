...
Сложность: 
- Время — O(log h) в среднем и O(h) в худшем (несбалансировано), где h — высота дерева
- Память — O(1)
## Добавление
...
> Простая мнемоника — сначала левый, потом правый повороты
##### Право-лево (Правый большой)
...
if (h(x.right) - h(x.left) == 2) {	// (Лево-)право
	y = x.left;
...
Цвета — лишь способ следить за балансом. Дерево может использовать перекраску элементов (замену бита цвета узла) либо уже описанные повороты

![[Pasted image 20251030195208.png]]
...
Случаи:
![[Pasted image 20251030201724.png]]

Конкретно перекрас узлов проходит цепочкой вверх к корню, т.к. новый узел (по умолчанию красный) красит своего родителя в чёрный, прародителя в красный, и так до корня, который всегда должен быть чёрным. Также, необходимо менять цвет так, чтобы соблюдать правило высоты (поддреревья чёрного цвета имеют одну и ту же высоту)

Так же при удалении: Удаляем узел, смотрим на родственные узлы и считаем высоты поддеревьев с чёрными корнями. Далее просто устраняем перевесы перекрасом, пока не будет достигнута одна и та же высота чёрных поддеревьев
...
Обход узлов (всегда!) в отсортированном порядке — лево, вершина, право
...
Обход узлов в порядке: вершина, левое поддерево, правое поддерево — асимптотика O(n) — обход по массиву размером n, добавление и удаление из стека n раз — n + n ≈ n

Пример:
```
8 3 1 6 4 7 10 14 13
```
### Постфиксный (postOrder)
