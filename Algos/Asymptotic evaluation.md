...
В некоторых языках программирования (Go, C, C++ особенно) используются указатели — переменные, прямо указывающие на область в памяти. Имея указатель в C/C++, можно сделать динамический массив, по которому можно двигаться по индексу в указателе `a[i]` (указатель `a = new int*[n]`)
